...
# A method to have one drone hovering in the center and two drones making a box on each side. All 3 flip at the end.
# Takes in length_of_side as a parameter to determine how large you want the box to be.
def Box(length_of_side):
    Drone1_do("cw 180", 4) #rotate 180 to face opposite from other two drones
    i = 0 # to make the box with 4 corners.
    while i < 5:
        if i == 0:
            send2(f"forward {length_of_side/2}",4) #move half the length to get it back to the center
            Drone3_do("stop",4) #so it wont turn off
        elif i == 4:  #if its first iteration we only want to move half the distance
            send2("cw 90", 4)  # turn right
            send2(f"forward {length_of_side/2}",4) #starts at the center so move half the length
            Drone3_do("stop",4) #so it wont turn off
        else:
            send2("cw 90", 4)  # turn right
            Drone3_do("stop",4) #so it wont turn off
        ...
        i += 1
    send("stop",4)
    Drone1_do("cw 180",4) #have them all face the same way again
    send("flip f",4) #flip all 3 drones forward
    time.sleep(2) #sleep for 2 seconds

send("command",4)
send("battery?",4)
send("takeoff",7)
print("Call to make boxes")
Box(100)
print("Completed box")
# Print message
